...
LaserdockDevicePrivate::LaserdockDevicePrivate(libusb_device *device, LaserdockDevice *q_ptr) : q(q_ptr),
                                                                                                usbdevice(device),
                                                                                                devh_ctl(NULL),
                                                                                                devh_data(NULL),
...
int LaserdockDevicePrivate::sdescription(char *buffer, int bufferSize)
{
    ...
    //  print our devices
    int bc = snprintf(buffer, bufferSize, "0x%04x 0x%04x", device_descriptor.idVendor, device_descriptor.idProduct);
    ...
    if (device_descriptor.iManufacturer)
    {
        ...
        bc += snprintf(buffer, bufferSize, "%s %s", buffer, manufacturer);
    }
